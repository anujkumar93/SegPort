...
//add image_seg_data layer from the previous version of deeplab
template <typename Dtype>
...
// add the image_mat_layer to read data from .Mat file
template <typename Dtype>
class ImageMatLayer : public ImageDimPrefetchingDataLayer<Dtype> {
 public:
  explicit ImageMatLayer(const LayerParameter& param)
    : ImageDimPrefetchingDataLayer<Dtype>(param) {}
  virtual ~ImageMatLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  ...
  //  return LayerParameter_LayerType_IMAGE_DATA;
  //}
  virtual inline const char* type() const { return "ImageMat"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int ExactNumTopBlobs() const { return 3; }
  virtual inline bool AutoTopBlobs() const { return true; }
