...
void Solver<Dtype>::Step(int iters) {
  ...
  for (; iter_ < stop_iter; ++iter_) {
    ...
    for (int i = 0; i < net_->params().size(); ++i) {
      ...
      //LOG(INFO)<<"get the computing mode.";
      switch (Caffe::mode()) {
      case Caffe::CPU:
        //LOG(INFO)<<"Using CPU to compute the diff...";      
        caffe_set(blob->count(), static_cast<Dtype>(0),
            blob->mutable_cpu_diff());
        //LOG(INFO)<<"end CPU computing...";
        break;
      case Caffe::GPU:
#ifndef CPU_ONLY
        //LOG(INFO)<<"Using GPU to compute the diff...";
        caffe_gpu_set(blob->count(), static_cast<Dtype>(0),
            blob->mutable_gpu_diff());
        //LOG(INFO)<<"end GPU computing...";
#else
        NO_GPU;
#endif
        break;
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  LOG(INFO)<<param_.max_iter()-iter_;
