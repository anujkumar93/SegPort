...
// Added by Xiaoyong Shen...
template<typename Dtype>
void DataTransformer<Dtype>::TransformMatSegImg(const double *pmatvarimg, int img_row, int img_col, int img_chs,
      const double *pmatvarimglabel, Blob<Dtype>* transformed_data_blob, 
      Blob<Dtype>* transformed_label_blob)
{
  
  int img_height   = img_row;
  int img_width    = img_col;
  int img_channels = img_chs;

  //const int seg_channels = 1;
  //int seg_height   = img_row;
  //int seg_width    = img_col;
  ...
  //CHECK_EQ(seg_channels, 1);
  CHECK_EQ(img_chs, data_channels);
  //CHECK_EQ(img_height, seg_height);
  //CHECK_EQ(img_width, seg_width);
  ...
  */
  ...
  //LOG(INFO)<<data_height<<" "<<data_width<<" "<<img_channels<<" "<<img_width<<" "<<img_height;
  ...
  for (int h = 0; h < data_height; ++h) {
    ...
    for (int w = 0; w < data_width; ++w) 
    {
      ...
      for (int c = 0; c < img_channels; ++c) 
      {
        ...
        if ((w+w_off)<0 ||(w+w_off)>img_width-1 || (h_off+h)<0 || (h+h_off)>img_height-1 ) 
        {
          pixel = 0;  // may have other methods....
        }
        else
        {
          int pixelidx = c*img_height*img_width + (w+w_off)*img_height + (h+h_off);
          pixel = static_cast<Dtype>(pmatvarimg[pixelidx]);
        ...
        /* do not set mean_values and mean files.... predefined in the .mat files...
        if (has_mean_file) 
        {
          int mean_index = (c * img_height + h_off + h) * img_width + w_off + w;
          transformed_data[top_index] =
      ...
      if ((w+w_off)<0 ||(w+w_off)>img_width-1 || (h_off+h)<0 || (h+h_off)>img_height-1 ) 
        {
          label = 0;  // may have other methods....
        }
        else
        {
          int pixelidx = (w+w_off)*img_height + (h+h_off);
          label = static_cast<Dtype>(pmatvarimglabel[pixelidx]);
          //LOG(INFO)<<label;
        }
      transformed_label[top_index] = label;
