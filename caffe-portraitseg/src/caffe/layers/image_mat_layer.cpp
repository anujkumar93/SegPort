//This is to implement the Mat Data Layer
// Added by Xiaoyong Shen 07/27/2015
...
ImageMatLayer<Dtype>::~ImageMatLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void ImageMatLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  //const int new_chanels = this->layer_param_.image_data_param().new_chanels();
  ...
  CHECK(transform_param.has_mean_file() == false) << 
         "ImageMatLayer does not support mean file";
  ...
  //should update the following code
  // Read an image, and use it to initialize the top blob.
  //cv::Mat cv_img = ReadImageToCVMat(root_folder + lines_[lines_id_].first,
  //                                 new_height, new_width, is_color);

  // Open a .mat file to read the channel informaiton
  mat_t *matfp;
  string fname = root_folder+lines_[lines_id_].first;
  matfp = Mat_Open((root_folder + lines_[lines_id_].first).c_str(), MAT_ACC_RDONLY);
  CHECK(matfp) << "Error opening MAT file " << fname;
  
  // Read data information
  matvar_t *matvar;
  matvar = Mat_VarRead(matfp,"img");

  CHECK(matvar) << "Field 'data' not present in MAT file " << fname;
  {
    CHECK(matvar->rank < 5) << "Field 'data' cannot have ndims > 4 in MAT file";
  }

  const int channels = matvar->dims[2];
  const int height = matvar->dims[0];
  const int width = matvar->dims[1];
  Mat_VarFree(matvar);
  Mat_Close(matfp);
...
template <typename Dtype>
void ImageMatLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void ImageMatLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int img_row, img_col, img_chs;

    mat_t *pmatimg = NULL;
    matvar_t *pmatvarimg = NULL;
    string fnameimg = root_folder + lines_[lines_id_].first;
    pmatimg = Mat_Open(fnameimg.c_str(),MAT_ACC_RDONLY);

    CHECK(pmatimg)<<"can not open the .mat file: "<<fnameimg;
    pmatvarimg = Mat_VarRead(pmatimg,"img");

    img_row = pmatvarimg->dims[0];
    img_col = pmatvarimg->dims[1];
    img_chs = pmatvarimg->dims[2];

    unsigned datasize = pmatvarimg->nbytes/pmatvarimg->data_size;
    CHECK_EQ(datasize,img_row*img_col*img_chs)<<"failed to load img: "<<fnameimg;

    const double *pimgdata = static_cast<const double *>(pmatvarimg->data);
    ...
    */
    ...
    // to read the label data

    int img_rowlabel, img_collabel;

    mat_t *pmatimglabel = NULL;
    matvar_t *pmatvarimglabel = NULL;
    string fnameimglabel = root_folder + lines_[lines_id_].second;
    pmatimglabel = Mat_Open(fnameimglabel.c_str(),MAT_ACC_RDONLY);

    CHECK(pmatimglabel)<<"can not open the .mat file: "<<fnameimglabel;
    pmatvarimglabel = Mat_VarRead(pmatimglabel,"mask");

    img_rowlabel = pmatvarimglabel->dims[0];
    img_collabel = pmatvarimglabel->dims[1];

    unsigned datasizelabel = pmatvarimglabel->nbytes/pmatvarimglabel->data_size;
    CHECK_EQ(datasizelabel,img_rowlabel*img_collabel)<<"failed to load img: "<<fnameimglabel;
    const double *pimgdatalabel = static_cast<const double *>(pmatvarimglabel->data);
    ...
    // Apply the new transform function
    this->data_transformer_->TransformMatSegImg(pimgdata, img_row, img_col, img_chs,
      pimgdatalabel, &(this->transformed_data_), &(this->transformed_label_));
    
    //Free the .Mat data
    Mat_VarFree(pmatvarimglabel);
    Mat_VarFree(pmatvarimg);

    Mat_Close(pmatimg);
    Mat_Close(pmatimglabel);
...
INSTANTIATE_CLASS(ImageMatLayer);
REGISTER_LAYER_CLASS(ImageMat);
}  // namespace caffe
